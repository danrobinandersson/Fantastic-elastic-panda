...
/*
The conent of this file should be entered into the Edge Function at supabase/functions/validate-and-payout/index.ts. 
This serves as the server-side logic for validating player performance and processing payouts based on blendshape matching scores.
Whenever this document is updated, the content should be copied into the Edge Function to keep the server-side logic in sync with the latest implementation.
...
/**
 * Remap raw score into wider visible score bands.
 * Keeps gameplay difficulty identical while improving score spread.
 */
...
/**
 * Calculate score from blendshape similarity.
 * Uses ORIGINAL gameplay difficulty.
 * Then remaps score visually for wider gaps.
 */
function scoreMatch(
...
/**
 * Determine payout multiplier from validated score.
...
console.log(`Validated payout: ${validatedPayoutAmount}x`);
 ...
 let payoutResult: { success: boolean; stamp?: Stamp; error?: string } =
  { success: false, error: "No payout requested" };
 if (validatedPayoutAmount > 0) {
